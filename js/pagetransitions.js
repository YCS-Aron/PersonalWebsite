var PageTransitions = (function() {
	...
	function init() {
		...
		// $iterate.on( 'click', clickFunc);

		$('.st-menu.st-effect-7 ul li').each(function(index, option) {		//the paramenters of callback of each is index and item
			var scope = this;
			option.onclick = function(mouseevent) {
				if( isAnimating ) {
					return false;
				}
				if( animcursor > 67 ) {
				...
				nextPage( animcursor, scope.childNodes[0].getAttribute( 'index' ) - '0');
				++animcursor;
			};
		});
	...
	function nextPage(options, targetPageIndex) {
		var animation = (options.animation) ? options.animation : options;	//the index of Animation type
